...
class JwtService {
    ...
    signAccessToken(userId: string): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            const options: SignOptions = {
                expiresIn: this.accessTokenExpireTime as SignOptions["expiresIn"],
                issuer: this.tokenIssuer,
                audience: userId,
            }

            JWT.sign(payload, this.accessTokenSecret, options, (err: any, token?: string) => {
                if (err || !token) {
    ...
    verifyAccessToken(req: any, res: any, next: any): void {
        ...
        JWT.verify(token, this.accessTokenSecret, async (err: any, payload: any) => {
            if (err) {
            ...
            try {
                ...
                if (blacklisted) {
                    const errorResponse = new ErrorResponse("FORBIDDEN", "Forbidden", ["Token is blacklisted"])
                    return ApiResponse.error(res, {
                        success: false,
                        error: errorResponse,
                    })
    ...
    signRefreshToken(userId: string): Promise<string> {
        return new Promise((resolve, reject) => {
            const payload = {}
            const options: SignOptions = {
                expiresIn: this.refreshTokenExpireTime as SignOptions["expiresIn"],
                issuer: this.tokenIssuer,
                audience: userId,
            }

            JWT.sign(payload, this.refreshTokenSecret, options, (err: any, token?: string) => {
                if (err || !token) {
                    logger.error(err?.message, err)
                    const errorResponse = new ErrorResponse("INTERNAL_SERVER_ERROR", "Internal Server Error")
                    return reject(errorResponse)
    ...
    verifyRefreshToken(refreshToken: string): Promise<string> {
        return new Promise((resolve, reject) => {
            JWT.verify(refreshToken, this.refreshTokenSecret, (err: any, payload: any) => {
                if (err) {
    ...
    checkAccessToken(accessToken: string): Promise<{ userId: string }> {
        return new Promise((resolve, reject) => {
            JWT.verify(accessToken, this.accessTokenSecret, (err: any, payload: any) => {
                if (err) {
                    const message = err.name === "JsonWebTokenError" ? "Unauthorized" : err.message
                    const errorResponse = new ErrorResponse("UNAUTHORIZED", message)
                    return reject(errorResponse)
