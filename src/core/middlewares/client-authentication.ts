...
export const clientAuthentication = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!clientToken) {
        logger.warn(`Unauthorized access attempt from IP: ${req.ip} - No client token provided`)
        return res.status(401).send("Unauthorized")
    }

    const [username, password] = Buffer.from(clientToken, "base64").toString().split(":")
    ...
    if (username === validUser && password === validPass) {
        ...
        logger.warn(`Forbidden access attempt from IP: ${req.ip} - Invalid credentials`)
