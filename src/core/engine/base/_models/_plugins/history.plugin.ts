...
const historyPlugin = <T extends Document>(schema: Schema<T>, options: { modelName: string }) => {
    const createHistoryEntry = async (
        ...
        const currentUserId = AsyncStorageService.getInstance().get("currentUserId")
    ...
    schema.pre<T>("save", async function (next) {
        ...
        const changes = this.isNew
            ? this.toObject()
            : this.modifiedPaths().reduce((acc: Record<string, any>, path: string) => {
    ...
    schema.methods.softDelete = async function (this: T & { deletedAt: Date | null }) {
        this.deletedAt = new Date()
        await this.save()
        const snapshot = this.toObject()
        await createHistoryEntry(this as Document, "softDelete", { deletedAt: this.deletedAt }, snapshot)
    }

    schema.methods.restore = async function (this: T & { deletedAt: Date | null }) {
        this.deletedAt = null
        await this.save()
        const snapshot = this.toObject()
        await createHistoryEntry(this as Document, "restore", { deletedAt: null }, snapshot)
    }

    schema.pre<T>("deleteOne", { document: true, query: false }, async function (next) {
        const snapshot = this.toObject()
    ...
    schema.pre("findOneAndUpdate", async function (next) {
        ...
        if (doc) {
            ...
            await createHistoryEntry(doc as Document, "update", changes as Record<string, any>, snapshot)
        }
        next()
    })
    ...
    schema.pre("updateMany", async function (next) {
        ...
        for (const doc of docs) {
            ...
            await createHistoryEntry(doc as Document, "update", updates as Record<string, any>, snapshot)
        }
        next()
    })
}
