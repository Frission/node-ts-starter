...
interface Config {
    ...
    otp: {
        length: number
        expiration: number
        purposes: Record<string, { code: string; title: string; description: string; message: string }>
    }
}
...
export const config: Config = {
    ...
    mail: {
        ...
        port: parseInt(
            process.env.NODE_ENV === "production" ? process.env.SMTP_PORT || "587" : process.env.MAILDEV_PORT || "1025",
            10,
        ),
        user: process.env.NODE_ENV === "production" ? process.env.SMTP_USER || "" : "",
        pass: process.env.NODE_ENV === "production" ? process.env.SMTP_PASS || "" : "",
