...
import { BaseRepository } from "../_repositories/base.repo"
...
export class BaseService<T extends Document, R extends BaseRepository<T>> {
    ...
    private async ensureUniqueField(doc: Partial<T>, field: keyof T): Promise<void> {
        ...
        if (exists) {
            throw new ErrorResponse("UNIQUE_FIELD_ERROR", `The ${String(field)} must be unique.`, [
    ...
    private async ensureUniqueSlug(
        ...
        do {
            ...
            if (exists) slug = `${slugify(doc[inputSlugField] as unknown as string)}-${++count}`
        } while (exists)
        ...
        try {
            ...
            for (const field of this.uniqueFields) {
                if (
                    ...
                    fieldsToUpdate[field as keyof T] !== documentToUpdate[field as keyof T]
                ) {
    ...
    async delete(query: Record<string, any>, softDelete = true): Promise<SuccessResponseType<T> | ErrorResponseType> {
        try {
            ...
            if (!deletedDocument) {
                throw new ErrorResponse(
                    "NOT_FOUND_ERROR",
                    softDelete ? "Document to soft delete not found." : "Document to delete not found.",
                )
            }
