...
class OTPRepository extends BaseRepository<IOTPModel> {
    ...
    async markAsUsed(otpId: string): Promise<IOTPModel | null> {
        return await this.model.findByIdAndUpdate(otpId, { used: true }, { new: true }).exec()
    }
    ...
    async findValidCodeByUser(code: string, user: string, purpose: TOTPPurpose): Promise<IOTPModel | null> {
    ...
    async invalidateOldCodes(user: string, purpose: TOTPPurpose): Promise<void> {
        await this.model.updateMany({ user, used: false, purpose }, { $set: { isFresh: false } }).exec()
    }
}
